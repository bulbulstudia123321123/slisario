function Snake(scene, size, color, gridSide) {
    ...
    this.snake.forEach(function (cube) {
      ...
      if (self.axis !== null && self.direction !== null) {
        if (!next) {
          ...
          if (self.position.x>self.gridSide || self.position.x<-self.gridSide || self.position.z>self.gridSide || self.position.z<-self.gridSide) {self.selfCollision()}
          if (self.tagPosition) {
